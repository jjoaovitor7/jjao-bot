    ...
    if (dailyAux != 0 && timeout - (Date.now() - daily) > 0) {
      ...
      message.channel.send(
        new Discord.MessageEmbed()
          .setColor("#0099ff")
          .addField("Moedas do Dia", qtde + " moedas.")
      );
      database
        .collection("Usuarios")
        .doc(message.guild.id)
  ...
  async weekly(database, message) {
    ...
    if (weeklyAux != 0 && timeout - (Date.now() - weekly) > 0) {
      ...
      message.channel.send(
        new Discord.MessageEmbed()
          .setColor("#0099ff")
          .addField("Moedas da Semana", qtde + " moedas.")
      );
      database
        .collection("Usuarios")
        .doc(message.guild.id)
  ...
  async monthly(database, message) {
    ...
    if (monthlyAux != 0 && timeout - (Date.now() - monthly) > 0) {
      ...
      message.channel.send(
        new Discord.MessageEmbed()
          .setColor("#0099ff")
          .addField("Moedas do Mês", qtde + " moedas.")
      );
      database
        .collection("Usuarios")
        .doc(message.guild.id)
